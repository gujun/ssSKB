...
import com.cari.voip.keyboard.soft.adapter.smsContentProvider;
import com.cari.voip.keyboard.soft.adapter.smsLabelProvider;
...
import com.cari.voip.keyboard.soft.model.smsFactory;
...
public class smsView extends ViewPart {

	public static final String ID_VIEW = 
		"com.cari.voip.keyboard.soft.views.smsView";
	...
	public void createPartControl(Composite parent) {
		...
		new TableColumn(this.tb,SWT.NONE).setText("发送者");
		
		layout.addColumnData(new ColumnWeightData(60));
		new TableColumn(this.tb,SWT.NONE).setText("接收者");
		
		layout.addColumnData(new ColumnWeightData(80));
		new TableColumn(this.tb,SWT.NONE).setText("内容");
		
		layout.addColumnData(new ColumnWeightData(30));
		new TableColumn(this.tb,SWT.NONE).setText("类型");
		
		layout.addColumnData(new ColumnWeightData(60));
		new TableColumn(this.tb,SWT.NONE).setText("发送时刻");
		
		this.tv.setLabelProvider(new smsLabelProvider());
		this.tv.setContentProvider(new smsContentProvider());
	...
	protected void onRecordingDoubleClick(DoubleClickEvent event){
		...
		if(o instanceof sms){
			sms rec = (sms)o;
			String from = rec.sms_from;
			
			if(rec.proto != null && rec.proto.length() > 0){
				from = rec.proto+":"+rec.sms_from;
			}
			
			String to = rec.sms_to;
			
			if(rec.chat != null && rec.chat.length() > 0){
				to = rec.chat+":"+rec.sms_to;
			}
			
			String type = (rec.sms_type != null && rec.sms_type.length()>0)?"类型：\t"+rec.sms_type+"\n":"  ";
			String tip = "from：\t"+from+"\n"+
						"to：\t"+to+"\n"+
						"时刻：\t"+rec.start_stamp+"\n"+type+
						"内容：\n\t"+rec.body;
	...
	protected void doQuerry(){
		...
		try {
			List recs = smsFactory.getSMS(this.getSession(), sql);
			tipShell.setVisible(false);
			refreshTable(recs);
			if(recs.size() > 0){
	...
	protected void onQuerryClick(){
		...
		this.sql = smsFactory.getQuerryString(this.start_time,
	...
	protected void onDeldeledClick(){
		if(MessageDialog.openConfirm(this.shell, "确认", "清除这些短信记录？")){
		try {
			
			smsFactory.deldeled(this.getSession(), 
					smsFactory.getDeldeledString(this.start_time, this.end_time, this.user_id));
			doQuerry();
		} catch (Exception e) {
